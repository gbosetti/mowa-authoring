/**
 * Represents a file in the filesystem with a name, an extension and some content. A
 * FilesManager instance returns a set of instances of this class through the
 * getFile(dirName, fileName) method. An instance of this class contains a nsiFile
 * that allow clients to access the name, extension and content so much easier than
 * with a nsIFile.
 * @constructor
 */
...
function JsonFileManager(){
	...
	this.getAuthoredAppsFiles = function(dirs){
		...
		//console.log("wrapped files", wrappedFiles);
	...
	this.getFiles = function(dirs){
		...
		for (var i = 0; i < nsiFiles.length; i++) {
			...
			try{
				...
				//console.log(nsiFiles[i].leafName, nsiFiles[i].path, cnt.toString());
